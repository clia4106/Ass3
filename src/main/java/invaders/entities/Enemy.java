...
public class Enemy extends BaseEntity implements GameObject, Moveable {
    ...
    public void dealColliding(GameEngine model, List<EntityView> entityViews,Renderable renderable) {
        if(renderable instanceof PlayerCannonball){
            ...
            if("slow_straight".equals(this.projectile)){
                model.getPlayer().addScore(3);
            }else if("fast_straight".equals(this.projectile)){
                model.getPlayer().addScore(4);
            }
        }
    }
    ...
    public void setRightMoved(boolean rightMoved) {
        isRightMoved = rightMoved;
    }
    ...
    public Renderable clone() {
        Enemy alien = new Enemy(new Vector2D(this.getPosition().getX(), this.getPosition().getY()),this.getProjectile());
        alien.setRightMoved(this.isRightMoved);
        alien.setSpeed(this.getSpeed());
        return alien;
    }
